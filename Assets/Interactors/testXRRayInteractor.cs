...
namespace UnityEngine.XR.Interaction.Toolkit
{
    ...
    public partial class XRRayInteractor : XRBaseControllerInteractor, IAdvancedLineRenderable, IUIHoverInteractor, IXRRayProvider, IXRScaleValueProvider
    {
        ...
        /// <inheritdoc />
        private protected void OnXRControllerChanged()
        {

            var testController = gameObject.GetComponentInParent<XRBaseController>(true);
            ...
            m_ScreenSpaceController = testController as XRScreenSpaceController;
            m_IsScreenSpaceController = m_ScreenSpaceController != null;
            ARDebugManager.Instance.LogInfo($"testController  : {testController}");

            ARDebugManager.Instance.LogInfo($"m_IsActionBasedController  : {m_IsActionBasedController} m_IsDeviceBasedController : {m_IsDeviceBasedController} m_IsScreenSpaceController : {m_IsScreenSpaceController}");
        ...
        protected override void Awake()
        {
            ...
            ARDebugManager.Instance.LogInfo("Awake");

            m_LocalPhysicsScene = gameObject.scene.GetPhysicsScene();
            //m_RegisteredUIInteractorCache = new UI.RegisteredUIInteractorCache(this);
        ...
        {
            ARDebugManager.Instance.LogInfo("OnDrawGizmosSelected");
...
#if AR_FOUNDATION_PRESENT 
            if (this.TryGetCurrentARRaycastHit(out var arRaycastHit))
            {
                // Draw the normal of the surface at the hit point
                Gizmos.color = new Color(58 / 255f, 122 / 255f, 248 / 255f, 237 / 255f);
                const float length = 0.075f;
        ...
        {
            ARDebugManager.Instance.LogInfo("DrawQuadraticBezierGizmo");
        ...
        public bool IsOverUIGameObject()
        {
            ...
            //&& m_RegisteredUIInteractorCache != null && m_RegisteredUIInteractorCache.IsOverUIGameObject();
        }
        ...
        protected virtual void RotateAnchor(Transform anchor, float directionAmount)
        {
            ...
            ARDebugManager.Instance.LogInfo($"AnchorRotateReferenceFrame here {m_AnchorRotateReferenceFrame}");
            ARDebugManager.Instance.LogInfo($"anchor here {anchor}");
            ...
            if (updatePhase == XRInteractionUpdateOrder.UpdatePhase.Dynamic)
            {
                ...
                if (m_AllowAnchorControl && hasSelection)
                {
                    if (m_IsDeviceBasedController && m_DeviceBasedController.inputDevice.isValid)
                    {
                        ...
                        {
                            ARDebugManager.Instance.LogInfo("ProcessInteractor inPressed");
        ...
        //public TrackableType trackableType => throw new NotImplementedException();
